...
def main(args):
    ...
    pdb_files = []
    root_txt_name = '/home/taofeng/ziqigao/PromptMSP_camera_ready/pre_process_results/new_test_set.txt'
    root_pdb_name = './pdb_all/'
    output_dir = '/mnt/sda/gaoziqi/dimer_testset/'

    with open(root_txt_name, "r") as f:  # 打开文件
        for line in f.readlines():
            pdb_files.append(line[:-1])

    # pdb_files = pdb_files[89:]
    chain_name_list = torch.load('/home/taofeng/ziqigao/PromptMSP_camera_ready/pre_process_results/ture_chain_name_list_new_test_set.pt')
    # chain_name_list = chain_name_list[89:]
    # chain_name_list = chain_name_list[110:]
    # pdb_files = pdb_files[110:]
    for ind, file in enumerate(tqdm(pdb_files)):
        ...
        for sing_com in tqdm(all_combine):
            ...
            if np.where(ligand_receptor_distance < 8)[0].shape[0] < ligand_receptor_distance.shape[0] * ligand_receptor_distance.shape[1] * 0.1 * 0.01:
                # print('extract')
